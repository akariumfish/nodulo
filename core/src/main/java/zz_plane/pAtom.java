...
import zz_patch.pMacro;
import zz_patch.pNode;
import zz_patch.pNodeSpace;
import zz_patch.pPar;
import zz_patch.pSheet;
import zz_patch.pStandard;
import zz_patch.pMacro.Macro;
import zz_patch.pMacro.MacroScript;
...
//		.addLink("reg_in_bp", "co_reg", "register", "co_reg")
		...
		.addRun(new nRun() { public void run() {
			...
			sValue v = list.get("construct2_ank").patch.plane.getSystem(pView.class)
				.bloc.getValue("val_grid");
			if (v != null) ((sBoo)v).set(false);
			v = list.get("construct2_ank").patch.plane.getSystem(pAtom.class)
		...
		pPlane.newStartupModel("atom_game")
...
//			pSheet.setDefCollapse("function", true);
	...
	public static void build_prop(Applet app) {
		...
		nRun run_ctrl_atom = new nRun() { public void run(Object o) { 
			...
			pAtom ato = bod.space.plane.getSystem(pAtom.class);
...
//		val_bullet_damage = bloc.obtainInt("val_bullet_damage", (int)1);
	...
	public void system_load() {
		...
		geo = plane.getSystem(pGeom.class);
