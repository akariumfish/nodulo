...
public class nModelBook {
	...
	private void build_book(boolean RELEASE) {
		...
		newModel("SZ1-1").setSize(1*RS, 1*RS);
		newModel("SZ1-2").setSize(1*RS, 2*RS);
		...
		.setOutlineWeight(3)
		;
		newModel("CL_release_CW")
		.copyFrom(getModel("CL_release"))
		.setOutlineWeight(3)
		;

		newModel("CL_def_CW_head")
		.copyFrom(getModel("CL_def"))
		.setFont(20)
		.setOutline(true)
		.setOutlineWeight(1)
...
//			app.DEF_TICK_BY_SEC = 60f;
			...
			newModel("CW_head_color").copyFrom(getModel("CL_release_CW_head"));
			newModel("text_field").copyFrom(getModel("CL_release_FLD"));
...
//			app.gdx.drawer.color_back = getModel("CL_def").color_background;
			...
			newModel("CW_head_color").copyFrom(getModel("CL_def_CW_head"));
			newModel("text_field").copyFrom(getModel("CL_def_FLD"));
			newModel("CL_graph").copyFrom(getModel("CL_def_graph"));
			newModel("CL_right_trigg").copyFrom(getModel("CL_def_RT"));
		}
		
		newModel("bp")
		.copyFrom(getModel("ref"))
		.copySizeFrom(getModel("SZ1-1"))
		.setTrigger()
		;
//
//		newModel("text_field")
...
//		;
//		
//		newModelGroup("tab_tab", new nModelGroup(app) { 
...
//		} );
//		
//		newModelGroup("tab_window", new nModelGroup(app) { 
//			public nWidgetGroup build(nGUI gui) {
...
//		.setHoverableZone(true)
		...
		newModelGroup("info_pop", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
				g.ref = "info_pop";
...
//							run_hide.run();		
		...
		.setStackSpacing(0)
		.setOutline(true)
		.setOutlineWeight(getModel("CL_CW").outlineWeight)
		.set_color_outline(getModel("CL_CW").color_outline)
		.setOutlineAfterChild(true)
		;
 
		
		newModel("CW_headback")
		.copyFrom(getModel("CW_head_color"))
//		.setRect(0,0,0,0)
//		.setBackground()
		.setBoundParent(true)
		.setBoundChild(true)
...
//		.set_color_outline(Utl.color(0,0,190))
		...
		.setHoverableZone(true)
		;
		newModel("CW_header_trigg")
		.copyFrom(getModel("CW_header_button"))
		.setTrigger()
		;
		newModel("CW_collapse")
		.copyFrom(getModel("CW_header_trigg"))
		.setText("_")
		;
		newModel("CW_close")
		.copyFrom(getModel("CW_header_trigg"))
		...
		newModelGroup("complex_window", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
				...
				g.addMetode("link_window_to_bloc", new nRun() {
					public void run(Object o) {
						...
						sVec val_pos = (v.getValue("val_pos") == null) ? 
								v.obtainVec("val_pos", getNewWindowPos()) :
								v.obtainVec("val_pos");
						ref.setLink(val_pos);
...
//		.setRect(0,0,RS*15f,RS*10f)
		...
		newModel("VP_center", new nModel(app) { public nWidget custom_init(nWidget w) { 
			w.setCustomDrawer(new nDrawable() { public void drawing() {
				...
				app.stroke(getModel("CL_VS_back").color_outline, 1f * scalefact);
				
				for (int i = 0 ; i < line_nb ; i++)
					for (int j = 0 ; j < line_nb ; j++) {
						if (j%2 == 0) app.stroke(getModel("CL_VS_back").color_outline, 
								0.5f * scalefact);
						else app.stroke(getModel("CL_VS_back").color_outline, 
								1f * scalefact);
...
//		.setRect(0,0,20,20)
		...
		newModelGroup("viewspace", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
...
//		.setRectOrigin(nAlign.LEFT,nAlign.TOP) // TOP   BOTTOM
		...
		newModelGroup("viewspace_tool", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
...
//					open.setText(s);
		...
		newModel("scrollist_slider")
		.copyFrom(getModel("ref"))
		.setRect(0,0,RS,RS*8)
		.setSlider()
		.setOutline(true)
		;
		newModel("scrollist_ref")
		.copyFrom(getModel("ref"))
		.setBoundChild(true)
		.setBoundParent(true)
		.setStacked(true)
...
//		.setOutlineWeight(1)
		;
		newModel("list_trigger")
		.copyFrom(getModel("list_entry"))
		.setTrigger()
		;
		newModel("list_entry_obj")
		.copyFrom(getModel("ref"))
		.setRect(0,0,RS,RS)
		.setStacked(true)
		...
		for (int i = 2 ; i <= 9 ; i++)
			...
			.copyFrom(getModel("list_entry_label_1")).setSX(RS*i);

		newModel("list_entry_trigg_1")
		.copyFrom(getModel("list_entry_obj"))
		.setSX(RS)
		.setTrigger()
		;
		for (int i = 2 ; i <= 9 ; i++)
			newModel("list_entry_trigg_"+i)
			.copyFrom(getModel("list_entry_trigg_1")).setSX(RS*i);

		newModel("list_entry_switch_1")
		.copyFrom(getModel("list_entry_obj"))
		.setSX(RS)
		.setSwitch()
		;
		for (int i = 2 ; i <= 9 ; i++)
			newModel("list_entry_switch_"+i)
			.copyFrom(getModel("list_entry_switch_1")).setSX(RS*i);
		
		newModelGroup("list", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
		...
		newModelGroup("scrollist", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
	...
	//					slider.setSliderVal(0f);	
		...
		newModelGroup("picklist", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
		...
		newModelGroup("treelist", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup("scrollist");
...
//		.setOutline(true)
		...
		newModelGroup("dropmenu", new nModelGroup(app) { 
			public nWidgetGroup build(nGUI gui) {
				nWidgetGroup g = gui.addWidgetGroup();
...
//		;
//
//		newModel("W_separator")
//		.copyFrom(getModel("W_entry"))
//		.setRect(0,0,150,5)
//		;
//		
//		newModel("W_trigg")
...
//		;
//		
//		newModelGroup("W_entry_flt_ctrl", new nModelGroup(app) { 
//			public nWidgetGroup build(nGUI gui) {
//				nWidgetGroup g = gui.addWidgetGroup();
//
...
//		} );
//		
//		newModelGroup("window", new nModelGroup(app) { 
//			public nWidgetGroup build(nGUI gui) {
//				nWidgetGroup g = gui.addWidgetGroup();
//				
