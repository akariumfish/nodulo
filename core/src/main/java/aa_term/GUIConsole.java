 ...
 * <a href="https://github.com/tommyettinger/sjInGameConsole">GitHub Repository</a> for more information.
 *
 * @author StrongJoshua
 */
public class GUIConsole extends AbstractConsole {
	...
	@Override public void resetInputProcessing () {
		...
		if (appInput != null) {
			...
			getMultiplexer().addProcessor(appInput);
	...
	private class ConsoleDisplay {
		...
		private TextButton submit, print;
		private Skin skin;
		...
		ConsoleDisplay (Skin skin) {
			...
			try {
				print = (TextButton) ClassReflection.getConstructor(textButtonClass, String.class, Skin.class)
						.newInstance(">>", skin);
			} catch (Exception e) {
				try {
					print = (TextButton) ClassReflection.getConstructor(textButtonClass, String.class)
							.newInstance(">>");
				} catch (Exception e2) {
					throw new RuntimeException(
						"TextButton class does not support either (<String>, <Skin>) or (<String>) constructors.");
...
//			refreshWindowColor();
		...
		void showPrint (boolean show) {
			print.setVisible(show);
			printCell.size(show ? print.getPrefWidth() : 0, show ? print.getPrefHeight() : 0);
		}
...
//			Utl.logn("keyL kDown "+input.getText());
			...
			if (keycode == keyID) {
				...
				input.setCursorPosition(input.getText().length());
//
//				Utl.logn("keyL kDown keyID "+input.getText());
//				
//				return false;
			}
...
//			Utl.logn("displayL kDown "+display.input.getText());
			
			if (disabled)
				return false;
			if (keycode == keyID) {
//				display.input.setText("");
//				display.input.setCursorPosition(display.input.getText().length());
				display.setHidden(!hidden);

//				Utl.logn("displayL kDown keyID "+display.input.getText());
