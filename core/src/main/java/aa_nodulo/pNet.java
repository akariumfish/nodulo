...
//		builder = builder(app, "net", pNet.class, !app.config.start_solo, new nRun() { public void run(Object o) {
//			sValueBloc b = (sValueBloc)o; newObject(b); }});
//	}

//	public static void dispose(App app) { pool.dispose(); }
...
//		plane.storeSystemType(bloc.ref, this.getClass());
		...
		if (app.config.start_as_server || app.config.start_as_client) 
			app.addDelayEvent(2, new nRun() { public void run() { 
				space = app.space;
				time = app.time;
				if (app.config.start_as_server && 
						app.config.AUTO_CONNECT) { init_server(); }
				else if (app.config.start_as_client && 
						app.config.AUTO_CONNECT) { init_client(); }
			}});
...
//				app.log("client pNet send client listening");
				...
				if (PRINT_LOG) Utl.logn("client pNet frame() could not sendClientSpeaking");
			}
		}
	}
...
//			app.log("server pNet send server listening");
			...
			if (PRINT_LOG) Utl.logn("server pNet tick() could not sendServerSpeaking");
		}
...
//			send_chg_collec(col); 
	...
	public void send_full_space() {
		...
		byte[] b = new byte[0];
		if (PRINT_LOG) Utl.logn("server pNet send bloc object change storing");
		net.sendPacket(BLOC_CHANGE, b);
		if (PRINT_LOG) Utl.logn("server pNet send clear space ");
		net.sendPacket(CLEAR_SPACE, b);
		for (pCollec col : space.collec_pool.all()) {
			if (PRINT_LOG) Utl.logn("server pNet send new collec "+col.pool_ref);
			send_new_collec(col); }
		for (Map.Entry<String, sPool<pParam>> me : space.param_pools.entrySet()) {
			...
			if (prop != null && !prop.mode_nosync) 
				for (pParam par : ppool.all()) {
					if (PRINT_LOG) Utl.logn("server pNet send new param "+par.pool_ref);
					send_new_param(par);
			}
		}
		for (pBody bod : space.body_pool.all()) {
			if (PRINT_LOG) Utl.logn("server pNet send new body "+bod.pool_ref);
			send_new_body(bod); }

		if (PRINT_LOG) Utl.logn("server pNet send unbloc object change storing ");
...
//	public static final int CHG_BODY_DATA = 16;
	...
	public void receiveClientPacket(int ref, byte[] data) {
		if (ref == CHG_COLLEC) {
			...
			if (b == null) { Utl.logn("ERROR : pNet receive client packet, CHG_COLLEC, pCollec b = null"); return; }
			if (PRINT_LOG) Utl.logn("server pNet received change collec "+b.pool_ref);
			...
			if (pool == null) { Utl.logn("ERROR : pNet receive client packet, CHG_PARAM, pool = null"); return; }
			pParam b = pool.getObjectAt(pool_id);
			if (b == null) { Utl.logn("ERROR : pNet receive client packet, CHG_PARAM, pParam b = null"); return; }
			if (PRINT_LOG) Utl.logn("server pNet received change param "+b.pool_ref);
			b.load_from_array(arr);
		} 
...
//		} 
	...
	public void receiveServerPacket(int ref, byte[] data) {
		if (ref == CLEAR_SPACE) {
			if (PRINT_LOG) Utl.logn("client pNet received clear space ");
			space.clear_all_obj();
		} else if (ref == BLOC_CHANGE) {
			if (PRINT_LOG) Utl.logn("client pNet received bloc object change storing ");
			space.bloc_change();
		} else if (ref == UNBLOC_CHANGE) {
			if (PRINT_LOG) Utl.logn("client pNet received unbloc object change storing, cancel stored change ");
			space.unbloc_change();
			space.cancel_change();
			...
			if (PRINT_LOG) Utl.logn("client pNet received new collec "+b.pool_ref);
			...
			b.init_from_array(arr);
			if (PRINT_LOG) Utl.logn("client pNet received new param "+b.pool_ref);
...
//			space.plane.getSystem(pGeom.class).init_body(b);
			if (PRINT_LOG) Utl.logn("client pNet received new body "+b.pool_ref);
			...
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received del body "+b.pool_ref);
			b.clear();
			...
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received del param "+b.pool_ref);
			b.clear();
			...
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received del collec "+b.pool_ref);
			b.clear();
...
//			String ref_in_prop = (String)rp;
			...
			pCollec b = space.collec_pool.getObjectAt(pool_id);
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received change collec "+b.pool_ref);
			b.load_from_array(arr);
			...
			pParam b = pool.getObjectAt(pool_id);
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received change param "+b.pool_ref);
			b.load_from_array(arr);
...
//		} 
			...
			if (b == null) return;
			if (PRINT_LOG) Utl.logn("client pNet received change body "+b.pool_ref);
			b.load_from_array(arr);
		} 
	}

	public void send_chg_body(pBody bod) {
		if (PRINT_LOG) Utl.logn("send chg body "+bod.pool_ref);
...
//	}
	public void send_chg_param(pParam par) {
		if (par.prop == null || par.prop.mode_localval) return;
		if (PRINT_LOG) Utl.logn("send chg param "+par.pool_ref);
	...
	public void send_chg_collec(pCollec col) {
		if (PRINT_LOG) Utl.logn("send chg collec "+col.pool_ref);
...
//		cnt += obj_to_stack(stack, col.ref_in_prop);
	...
	public void send_new_body(pBody bod) {
		if (PRINT_LOG) Utl.logn("send new body "+bod.pool_ref);
		Object[] arr = new Object[bod.data_size()];
		bod.to_array(arr);
	...
	public void send_new_param(pParam par) {
		if (PRINT_LOG) Utl.logn("send new param "+par.pool_ref);
		Object[] arr = new Object[par.data_size()];
		par.to_array(arr);
	...
	public void send_new_collec(pCollec col) {
		if (PRINT_LOG) Utl.logn("send new collec "+col.pool_ref);
		Object[] arr = new Object[col.data_size()];
		col.to_array(arr);
	...
	public void send_del_body(pBody bod) {
		if (PRINT_LOG) Utl.logn("send del body "+bod.pool_ref);
		ArrayList<byte[]> stack = new ArrayList<byte[]>(); int cnt = 0;
		cnt += obj_to_stack(stack, bod.pool_index);
		send_stack(DEL_BODY, cnt, stack); 
	}
	public void send_del_param(pParam par) {
		if (PRINT_LOG) Utl.logn("send del param "+par.pool_ref);
		ArrayList<byte[]> stack = new ArrayList<byte[]>(); int cnt = 0;
		cnt += obj_to_stack(stack, par.prop.ref);
		cnt += obj_to_stack(stack, par.pool_index);
	...
	public void send_del_collec(pCollec col) {
		if (PRINT_LOG) Utl.logn("send del collec "+col.pool_ref);
		ArrayList<byte[]> stack = new ArrayList<byte[]>(); int cnt = 0;
		cnt += obj_to_stack(stack, col.pool_index);
