...
public class pCommande {
	...
	public static pCommande newCommande(String r) {
		if (commandes.hasKey(r)) {
			app.logn("ERROR: cant create commande, <"+r+"> allready exist"); return null; }
...
//		if (!test_args(v)) return;
		if (!test_args(v)) app.logn("WARNING: cCommande "+ref+" .run(..) bad arguments");
...
//		if (cl != return_class) return null;
		if (cl != return_class) app.logn("WARNING: cCommande "+ref+" .run(..) bad return class");
//		if (!test_args(v)) return;
		if (!test_args(v)) app.logn("WARNING: cCommande "+ref+" .run(..) bad arguments");
