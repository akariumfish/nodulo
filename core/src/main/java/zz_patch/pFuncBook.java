...
import zz_patch.pFunc.C;
import zz_patch.pFunc.Instruction;
import zz_patch.pFunc.Operator;
import zz_patch.pTile.CT;
import zz_patch.pTile.PlugDef;
import zz_patch.pTileHead.Script;
import zz_patch.pTileHead.ScriptCom;
...
public class pFuncBook {
	...
	public static void build_operators(Applet app) {
...
//		}});
		...
		new Operator("get_input", "I", C.GETI, new nRun() {public Object get() {
			String in_ref = ask("in_ref", String.class);
				nRun in = instance.patch.plane.inputs.get(in_ref);
		...
		.setStandRun(new nRun() {public void run() {
			...
			.param("run_right", new nRun() {public void run() {
				...
				for (String par : instance.patch.plane.inputs.allKey()) {
...
//			app.logn("obj:"+obj+" mem:"+mem);
	...
	public static void build_instructions(Applet app) {	
		...
		new Instruction("set_output", "O", C.SETO, new nRun() {public Object get() {
			...
			nRun out = instance.patch.plane.outputs.get(out_ref);
			Object r = ask("data");
			if (r != null && out != null) { 
		...
		.setStandRun(new nRun() {public void run() {
			...
			.param("run_right", new nRun() {public void run() {
				...
				if (triggP_w == null) return;
				instance.patch.patch_dropmenu.metode("clear_entrys");
				for (String par : instance.patch.plane.outputs.allKey()) {
					nWidget w1 = (nWidget)instance.patch.patch_dropmenu
							.metodeGet("add_entry_custom", par, RS*6f, RS*2f/3f);
					w1.addEventTrigger(new nRun(instance) { public void run() {
