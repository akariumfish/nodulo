...
//			pBody bod = (pBody)o; if (bod == null) return;
//			pTime time = bod.space.plane.getSystem(pTime.class);
...
//		if (!app.RELEASE) 
	...
	public void frame(float delta) { 
		...
		if (val_do_draw.get() && plane.getSystem(pView.class).mouse_is_hover_view()) {
			boolean found = false;
			for (pBody b : space.familyMember("aabb_clickable")) 
...
//		dmg_zones.clear();
	...
	public boolean test_clic(pBody b) {
		...
		m.set(plane.getSystem(pView.class).mouse_in_view());
...
//		if (intersect) app.polygon(poly_overlap);
	...
	public void draw_body(Applet app, pBody b) {
		if (!b.hasParam("ref") || !b.hasParam("graph") || 
				!b.hasParam("geom")) return;
		draw_geom_graph(app, b, b.param("geom"), b.param("graph"));
	}

	public static void draw_geom_graph(Applet app, pBody b, pParam geom, pParam graph) {
		if (geom == null) return;
		ArrayList<Vector2> point = geom.getCollecData("point", Vector2.class);
