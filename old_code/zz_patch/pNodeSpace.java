...
import zz_patch.pMacro.Macro;
import zz_patch.pNode.CT;
import zz_plane.pBody;
import zz_plane.pGeom;
import zz_plane.pParam;
import zz_plane.pProperty;
	...
	public static void build_sheet(Applet app) {
		...
		.addEventLoad(new nRun() { public void run(Object o) {
			...
			nRun run_frame = new nRun(sheet) {public void run() { 
				...
				pSpace space = sheet.patch.plane.getSystem(pSpace.class);
...
//				}
	...
	public static void build_blueprint(Applet app) {
...
//		.setCommon()
	...
	public static void build_nodes(Applet app) {
		...
		.useLoad().commande(new nRun() { public void run() {
			pSpace space = instance.patch.plane.getSystem(pSpace.class);
			...
			space.addEventSpaceStart(run_space_start);
		}})
		.useClear().commande(new nRun() { public void run() {
			pSpace space = instance.patch.plane.getSystem(pSpace.class);
			space.removeEventSpaceStart(instance.object("run_space_start", nRun.class));
		}}).useInit()
		.openSec()
...
//			if (!prop.mode_common) continue;
	...
	private static pStandard build_prop_node(Applet app, String r, pProperty prop) {
		...
		stand.newRun("defParam", new nRun() {public void run() { 
			...
			pSpace space = instance.patch.plane.getSystem(pSpace.class);
			pParam par = space.getParam(prop.ref, par_ref);
			if (par == null) return; 
...
//			instance.patch.addEventFrame(run_frame);
		...
		.useLoad().commande(new nRun() {public void run() { 
			app.addDelayEvent(1, new nRun(instance) { public void run() {
				pInstance inst = (pInstance)builder;
				pSpace space = inst.patch.plane.getSystem(pSpace.class);
...
//				}
		...
		.openSec()
			.param("run", new nRun() {public void run() {
				...
				pSpace space = instance.patch.plane.getSystem(pSpace.class);
				instance.patch.patch_dropmenu.metode("clear_entrys");
				for (pParam param : space.param_pools.get(prop.ref).all()) {
...
//			interf.add_row();
	...
	public static void build_sel_body_node(Applet app) {
		...
		stand_sel_body.newRun("select_body", new nRun() {public void run() {
			...
			pSpace space = instance.patch.plane.getSystem(pSpace.class);
			if (space == null) return;
			pBody bod = space.body_pool.get(bod_ref);
			if (bod == null) return;
			
			if (bod.hasParam("owner") && bod.getBoo("owner", "owned") && 
					!bod.getStr("owner", "owner").equals(app.player_ref)) return;
			...
			pGeom geom = instance.patch.plane.getSystem(pGeom.class);
...
//					ank.run("metode", "set_default");
		...
		.newRun("unselect_body", new nRun() {public void run() {
			pSpace space = instance.patch.plane.getSystem(pSpace.class);
			if (space == null) return;
			pBody bod = space.body_pool.get(instance.getVar("body_ref", String.class));
			if (bod == null) return;
			if (bod.hasParam("highlightable")) 
...
//					ank.setVar("view_ank", false);
		...
		.newRun("do_prev_tick", new nRun() {public void run() { 
			if (instance.getVar("track", Boolean.class)) {
				...
				if (ank == null) return;
				pSpace space = instance.patch.plane.getSystem(pSpace.class);
				if (space == null || instance.getVar("body_ref", String.class) == null) return;
				pBody bod = space.body_pool.get(instance.getVar("body_ref", String.class));
		...
		.useLoad().commande(new nRun() {public void run() {
			app.addDelayEvent(1,new nRun(instance) { public void run() {
				pInstance inst = ((pInstance)builder);
				pSpace space = inst.patch.plane.getSystem(pSpace.class);
				if (space == null || inst.getVar("body_ref", String.class) == null) return;
				pBody bod = space.body_pool.get(inst.getVar("body_ref", String.class));
				if (bod != null) inst.run("select_body", bod.pool_ref); }});
		}}).useInit()
		.useClear().commande(new nRun() {public void run() {
			pTime time = instance.patch.plane.getSystem(pTime.class);
			time.removePrevTickBric(instance);
		...
		.openSec()
			.param("run", new nRun() {public void run() {
				...
				if (space == null) return;
				instance.patch.patch_dropmenu.metode("clear_entrys");
				for (String par : space.body_pool.allKey()) {
					nWidget w1 = (nWidget)instance.patch.patch_dropmenu
							.metodeGet("add_entry_custom", par, RS*6f, RS*2f/3f);
					w1.addEventTrigger(new nRun(instance) { public void run() {
			...
			.param("run", new nRun() {public void run() {
				...
				if (instance.getVar("owned", Boolean.class)) {
					if (bod.hasParam("owner") && !bod.getBoo("owner", "owned")) {
						...
						bod.setStr("owner", "owner", app.player_ref);
					} else if (bod.hasParam("owner") && bod.getBoo("owner", "owned") && 
							!bod.getStr("owner", "owner").equals(app.player_ref)) {
						instance.setVar("owned", false);
		...
		.openSec()
			.param("offer", new nRun() {public Object get() {
				pInstance node = instance.object("node", pInstance.class);
				pSpace space = instance.patch.plane.getSystem(pSpace.class);
				if (space == null || node.getVar("body_ref", String.class) == null) 
					return null;
				return space.body_pool.get(node.getVar("body_ref", String.class));
			}})
			.param("keys", new String[] {"body"}, "filters", new String[] {"body"}) 
	...
	//				app.log("sel bod received");
				pInstance node = instance.object("node", pInstance.class);
				pSpace space = instance.patch.plane.getSystem(pSpace.class);
				if (space == null || node.getVar("body_ref", String.class) == null) return;
				pBody bod = space.body_pool.get(node.getVar("body_ref", String.class));
				if (bod == null) return;
				String par_ref = arg(0,String.class);
	...
	//						ank.setVar("title", "Selected Body");
		...
		.openSec()
			.param("run", new nRun() {public void run() {
				...
				pSpace space = instance.patch.plane.getSystem(pSpace.class);
				if (space == null) return;
				pBody bod = space.body_pool.get(head.getVar("body_ref", String.class));
				if (bod == null) return;
				instance.patch.patch_dropmenu.metode("clear_entrys");
		...
		.openSec()
			.param("offer", new nRun() {public Object get() {
				...
				pSpace space = node.patch.plane.getSystem(pSpace.class);
				if (space == null) return null;
				pBody bod = space.body_pool.get(head.getVar("body_ref", String.class));
