...
public class pFunc {
	...
	public static void build_tile(Applet app) {
			...
			nRun ins_run = new nRun() {public Object get() {
				...
				public Object get() {
					...
					if (!ins.data_type.hasKey(r)) {
						Applet.loggn("ERROR : pFunc.build_tile.ins_run : "
								+ "ins dont have key "+r); 
						return null; } 
...
//			instance.getInst("tile_node").run("reset_next_id");
			...
			if (as == null) {
				Applet.loggn("ERROR : Tile branch_start get_full_branch_script : "
...
//		instance.getInst("tile_node").run("reset_next_id");
		...
		if (run_out != null && run_out.getInst("plugged") != null) {
			...
			while (pass && next != null && cnt < 500) {
				...
				if (as != null) {
					...
					Applet.loggn("ERROR : Tile stack_start get_instruction_script : "
			...
			if (cnt > 498) Applet.loggn("WARNING : pTile stack_start.run(get_instruction_script) : "
	...
	public static Object[] get_instruction_script(pInstance inst) {
		...
		if (ins == null) {
			Applet.loggn("ERROR : pFunc.get_instruction_script : "
					+ "cant find ins");
			return null; }
...
//			Data ma = ins.args[i];
			if (instance.get("get_plug", pInstance.class, arg_ref) != null && 
				...
				if (as != null) {
					...
					Applet.loggn("ERROR : Tile "+ins.ref+" get_instruction_script : "
	...
	public static Object[] get_branch_script(pInstance inst) {
		...
		if (ope == null) {
			Applet.loggn("ERROR : pFunc.get_branch_script : "
					+ "cant find ope");
			return null; }
		
//		Applet.loggn(""+instance.pool_ref+" "+ope.ref+" make script");
...
//				Applet.loggn(""+instance.pool_ref+" "+ope.ref+" do get script at "+
//						arg_ref+" for "+plug_tile.pool_ref);
				...
				if (as != null) {
					...
					Applet.loggn("ERROR : Tile "+ope.ref+" get_branch_script : "
							+ "the branch <"+arg_ref+"> did not return a script"); 
				}
			} else {
//				Applet.loggn("WARNING : Tile "+ope.ref+" get_branch_script : "
//						+ "the branch <"+ma.ref+"> is unplugged"); 
	...
	public static void func_script_run(pInstance inst, Object[] sc, Object[] passed_arg) {
		...
		if (script.size() + 2 != tot_size) { Applet.loggn("ERROR : pFunc.func_script_run() : "
		...
		if (cnt > 498) Applet.loggn("WARNING : pFunc func_script_run : "
				+ "looped throug too many instruction and stopped ");
	}
	...
	public static C instruction_script_run(
		...
		if (!virt.pop_result_from_script(inst, script, passed_arg)) { 
			Applet.loggn("ERROR : pFunc.instruction_script_run : "
...
//		Utl.logg("instruc: "+code.name()+" "+ins_id);
	...
	public static Object branch_script_obtain(
		...
		if (!virt.pop_result_from_script(inst, script, passed_arg)) { 
			Applet.loggn("ERROR : pFunc.branch_script_obtain : "
	...
	public static class Virtual implements nPool.Poolable {
		...
		public boolean pop_result_from_script(
			...
			if (script == null || script.size() == 0) {
				Applet.loggn("ERROR : pFunc.Virtual.pop_result_from_script : "
						+ "no script"); return false; }
			if (popC(script) != C.INT) {
				Applet.loggn("ERROR : pFunc.Virtual.pop_result_from_script : "
						+ "no script size"); return false; }
			int tot_size = pop(script, Integer.class);
			if (script.size() + 2 != tot_size) { Applet.loggn("ERROR : "
...
//					Applet.loggn("WARNING : pFunc.Virtual.pop_result_from_script : "
//							+ "found null arg");
...
//						Applet.loggn("WARNING : pFunc.Virtual.obtain_args_from_branchs : "
...
//					} 
			...
			if (pop_cnt != scr.arg_nb) {
				Applet.loggn("ERROR : pFunc.Virtual.obtain_args_from_branchs : "
