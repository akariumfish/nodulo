...
//		plane.app.log("cInstance.init(stand,args)");
	...
	public void from_tab(sTab t, int c, HashMap<String,String> map) {
		...
		if (stand == null) { 
			patch.Utl.logn("ERROR pInst from_tab cant find stand < "+stand_ref+" >"); 
			return; }

		int row_height = t.height(c);
		if (row_height < stand.data_size()) {
			patch.Utl.logn("ERROR : "+pool_ref+" from_tab : tab row too small : tab:"+row_height+" stand.datasize:"+stand.data_size()); }
...
//		patch.stand_insts.get(stand.ref).add(this);
		...
		if (row_height != stand.data_size()+(var_nb * 3)) {
			patch.Utl.logn("ERROR : "+pool_ref+" from_tab : tab row too small : "
	...
	public pColl obtainCollec(int i, String r) {
		...
		if (sheet.collec_pool.get(r) != null) {
			...
			if (p != null) {
				...
				patch.Utl.logn("ERROR: pInstance could not obtain a collec");
			}
			return p;
		}
	}
	...
	public pInstance addVar(String ref, Object var) {
		if (var_vals.hasKey(ref)) {
			stand.Utl.logn("ERROR : cInstance "+this.pool_ref+" has allready the key "+ref);
			return this; }
